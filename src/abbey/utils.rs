...
where
    T: Fn(Vec<S>) -> W,
    S: std::str::FromStr + std::clone::Clone,
    W: std::fmt::Display,
    <S as std::str::FromStr>::Err: 'static + std::error::Error,
{
    ...
    for i in 0..n {
        results.push(logic(
            parsed_input[1 + i * test_size..1 + (i + 1) * test_size].to_vec(),
        ));
    }

    let print_str = Iterator::reduce(results.iter().map(|num| num.to_string()), |s, t| {
        s + " " + &t
    })
    .ok_or("")?;
